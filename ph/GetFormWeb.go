...
func GetAllFromWeb(base string) {
	for i := 1; i <= 9; i++ {
		if i == 1 {
			...
			GetFromWeb(uri, strconv.Itoa(1))
			fmt.Println(uri)
			continue
		}
		uri := strings.Join([]string{base, "/videos?page=", strconv.Itoa(i)}, "")
		fmt.Println(uri)
		GetFromWeb(uri, strconv.Itoa(i+1))
		time.Sleep(3 * time.Second)
	}
}
func GetFromWeb(uri, index string) {
	...
	links, _ := os.OpenFile(strings.Join([]string{index, "txt"}, "."), os.O_APPEND|os.O_CREATE|os.O_WRONLY, 0777)
	defer links.Close()
	as := root.FindAll("a")
	for _, a := range as {
		href := a.Attrs()["href"]
		if strings.Contains(href, "viewkey=") {
			link := strings.Join([]string{"https://www.github.com", href}, "")
			fmt.Println(link)
